...
def player_fun(name):
    ...
    cur = conn.execute("""
        WITH all_matches AS (
            -- Every main-draw meeting between this player and any opponent,
            -- capturing both sides of the match in one pass.
            SELECT
                CASE WHEN m.winner_id = ? THEN m.loser_id
                     ELSE m.winner_id END                          AS opp_id,
                -- did the subject player win this match?
                CASE WHEN m.winner_id = ? THEN 1 ELSE 0 END       AS player_won,
                -- sets played = spaces in score string + 1
        ...
        h2h_stats AS (
            ...
            SELECT
                ...
                COUNT(*) - SUM(player_won)                   AS player_losses,
                -- closeness: avg fraction of possible sets that were actually played
                AVG(CAST(sets_played AS REAL) / best_of)     AS avg_closeness,
                AVG(jday)                                     AS avg_jday,
                MIN(jday)                                     AS min_jday,
                MAX(jday)                                     AS max_jday
            FROM all_matches
            GROUP BY opp_id
            HAVING total_matches >= 10
        ),
        normalised AS (
            -- Normalise each dimension to 0-1 across all qualifying rivals
            ...
            SELECT
                ...
                avg_closeness,
                avg_jday,
                -- normalise match count
                CASE WHEN MAX(total_matches) OVER () > MIN(total_matches) OVER ()
                     THEN (total_matches - MIN(total_matches) OVER ()) * 1.0
                          / (MAX(total_matches) OVER () - MIN(total_matches) OVER ())
                     ELSE 1.0 END                             AS norm_matches,
                -- closeness already 0-1 (fraction of sets played)
                avg_closeness                                 AS norm_closeness,
                -- normalise recency
                CASE WHEN MAX(avg_jday) OVER () > MIN(avg_jday) OVER ()
                     THEN (avg_jday - MIN(avg_jday) OVER ()) * 1.0
                          / (MAX(avg_jday) OVER () - MIN(avg_jday) OVER ())
                     ELSE 1.0 END                             AS norm_recency,
                -- weighted rival score
        ...
        SELECT
            ...
            n.player_wins                       AS wins,
            n.player_losses                     AS losses,
